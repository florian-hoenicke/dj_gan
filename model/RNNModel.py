...
def create(sequence_length=100, max_tones=12, max_octaves=20, rnn_units=256, hidden_dense_size=100, tones_rnn_dim=32, octaves_rnn_dim=32):
    instrument_type_in = tf.keras.layers.Input(shape=(sequence_length,), name='instrument_type')
    is_drum_in = tf.keras.layers.Input(shape=(sequence_length,), name='is_drum')

    instrument_type_enbedding = tf.keras.layers.Embedding(150, output_dim=20, name='instrument_embedding')(instrument_type_in)
    ...
    durations_in, velocities_in, tones_in, octaves_in = [tf.keras.layers.Input(shape=(sequence_length,), name=name) for name in ['durations_in', 'velocities_in', 'tones_in', 'octaves_in']]
    durations_reshaped, velocities_reshaped = [tf.keras.layers.Reshape(target_shape=(sequence_length,1))(x) for x in [durations_in, velocities_in]]
    ...
    tones_out = tf.keras.layers.Dense(units=max_tones, name='tones_out')(hidden1)
    octaves_out = tf.keras.layers.Dense(units=max_octaves, name='octaves_out')(hidden1)
    ...
    model.compile(
        ...
        loss={
            ...
            'tones_out': tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
            'octaves_out': tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        }
    )
    return model